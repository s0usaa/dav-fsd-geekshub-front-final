...
import { newTrack } from "../../services/apiCalls";
export const CreateTracks = () => {
  ...
  const matchCreate = () => {
    newTrack(createTracks, credencialesRdx.credentials.token)
      .then((resultado) => {
        setCreateTracks(resultado.data);
        setWelcome(
          `Pista numero ${createTracks.track_number} creada correctamente`
        );
        setTimeout(() => {
          navigate("/tracks");
        }, 2500);
      ...
      {welcome !== "" ? (
        ...
        <Row className="matchesDesign d-flex align-items-center justify-content-center">
          <Col xs={10} lg={6} sm={12} className="matchesDiv">
            <h1 className="mb-4 text-center">Crear una pista</h1>
            <Form>
